...
app.get('/:id', async (req, res, next) => {
    try {
        const result = await db.query("SELECT * FROM vendor_c WHERE id = $1", [req.params.id]);
...
app.post('/', async (req, res, next) => {
    ...
    try {
        const sql = `
            INSERT INTO vendor_c (id, details, pricing, stock) VALUES ($1, $2, $3, $4) RETURNING *
        `;
        const result = await db.query(sql, [
            id, details, pricing, stock
        ]);
        res.status(201).json(result.rows[0]);
    } catch (err) {
        next(err);
...
app.put('/:id',authenticateToken, async (req, res, next) => {
    ...
    try {
        const sql = `
            UPDATE vendor_b SET id=$1, details=$2, pricing=$3, stock=$4
            WHERE id=$5 RETURNING *
        `;
        const result = await db.query(sql, [id, details, pricing, stock, req.params.id]);
